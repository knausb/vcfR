...
#' data frames.
#'
#' A long, tidy dataframe based on a VCF annotated by \href{http://pcingola.github.io/SnpEff/}{snpEff} or
#' \href{https://www.ensembl.org/info/docs/tools/vep/index.html}{VEP} tools will still contain non-tidy annotation
#' columns. Those can be tidied by additional functions - see \link{vcfR_to_tidy_separation}
#'
...
#' @title Separate annotation columns in a tidy VCF
#' @description Some VCF annotation tools are storing the variant annotations in INFO column of the VCF file.
...
#'
#' @details Let \code{vcf_tidy} be the value (named list) returned from the \code{\link{vcfR2tidy}} function and
...
#'
#' @author Jiri Novotny <jiri.novotny@@img.cas.cz>
#'
#' @importFrom rlang .data .env
#' @name vcfR_to_tidy_separation
#' @rdname vcfR_to_tidy_separation
NULL

#' @param info A character scalar: description of field names from which column names will be deducted.
#' @param regex A character scalar: regular expression specifying a group in \code{info} containing the field names.
...
#' @param clean_names A logical scalar: if \code{TRUE}, the extracted column names are cleaned by
#'   \code{\link[janitor]{make_clean_names}}.
#' @param custom_names A named list: used to define custom column renamings.
...
#' @rdname vcfR_to_tidy_separation
#' @export
extract_ann_cols <- function(info, regex, clean_names = TRUE, custom_names = NULL, ...) {
  m <- stringr::str_match(info, regex)
  if (all(is.na(m))) {
    warning(glue::glue("No matches found for regex = '{regex}'"))
    return(NULL)
  } else if (!"cols" %in% colnames(m)) {
    warning(glue::glue("Regex named group 'cols' not found in the regex match."))
    return(NULL)
  }

  cols <- m[, "cols", drop = TRUE] %>%
    stringr::str_split(stringr::fixed("|")) %>%
    unlist() %>%
    stringr::str_trim()

  if (clean_names) {
    cols <- janitor::make_clean_names(cols)
  }

  if (!is.null(custom_names)) {
    cols <- dplyr::recode(cols, !!!custom_names)
  }

  return(cols)
}

#' @param meta A tibble: \code{meta} item in the list returned from \code{vcfR2tidy()}.
#'
#' @examples
...
#' @rdname vcfR_to_tidy_separation
#' @export
extract_ann_cols_snpeff <- function(meta, ...) {
  regexes <- c(
    "ANN" = "^Functional annotations: '(?<cols>.+)'",
    "LOF" = "Predicted loss of function effects for this variant. Format: '(?<cols>.+)'",
    "NMD" = "Predicted nonsense mediated decay effects for this variant. Format: '(?<cols>.+)'"
  )
  regexes <- regexes[names(regexes) %in% meta$ID]

  res <- purrr::map2(names(regexes), regexes, function(field, regex) {
    extract_ann_cols(
      meta %>% dplyr::filter(.data$ID == .env$field) %>% dplyr::pull(.data$Description),
      regex,
      ...
    )
  }) %>%
    magrittr::set_names(names(regexes)) %>%
    purrr::discard(is.null)

  if ("LOF" %in% names(regexes)) {
    res$LOF <- paste0("lof_", res$LOF)
  }

  if ("NMD" %in% names(regexes)) {
    res$NMD <- paste0("nmd_", res$NMD)
  }
...
#' @rdname vcfR_to_tidy_separation
#' @export
extract_ann_cols_vep <- function(meta, ...) {
  list(
    CSQ = extract_ann_cols(
      meta %>% dplyr::filter(.data$ID == "CSQ") %>% dplyr::pull(.data$Description),
      "Consequence annotations from Ensembl VEP. Format: (?<cols>.+)",
      ...
    )
  )
}

#' @param df A tibble: should be the \code{fix} item in the list returned from \code{vcfR2tidy()}, or \code{dat} item
#'   if \code{single_frame = TRUE} was used.
#' @param ann_col A character scalar: name of column to separate.
#' @param into_cols A character vector: names of new columns into which \code{ann_col} will be separated.
#' @param field_sep A character scalar: separator of annotation fields.
#' @param multi_sep A character scalar: separator of multiple annotations.
#' @param value_fn A function to apply to the \code{ann_col} column before the column-wise separation takes place.
#'   It must take two parameters \code{df} and \code{ann_col}, and return a dataframe-like object. Set to \code{NULL}
#'   to disable.
#' @param add_rank A logical scalar: if \code{TRUE}, each of per-variant annotations will be ranked (\code{ann_rank} column)
...
#' @param return_into_cols A logical scalar: if \code{TRUE}, only return the newly separated columns, otherwise the whole
#'   \code{df}.
#'
#' @examples
#' data("vcfR_test_VEP")
...
separate_ann <- function(df,
  ...
  if (!is.null(multi_sep)) {
    df <- df %>%
      tidyr::separate_rows(dplyr::all_of(ann_col), sep = multi_sep)
  }

  if (!is.null(value_fn)) {
    df <- value_fn(df, ann_col)
    stopifnot("value_fn() didn't return a dataframe object" = inherits(df, "data.frame"))
  }

  df <- tidyr::separate(df, ann_col, into_cols, sep = field_sep, remove = FALSE)

  ## -- See http://pcingola.github.io/SnpEff/se_inputoutput/#multiple-annotations-per-vcf-line
  if (add_rank) {
    df <- df %>%
      dplyr::group_by(.data$CHROM, .data$POS, .data$ID, .data$REF, .data$ALT) %>%
      dplyr::mutate(ann_rank = seq_len(dplyr::n())) %>%
      dplyr::ungroup()
  }

  if (return_into_cols) {
    df <- dplyr::select(df, dplyr::all_of(into_cols))
  }
...
#'
#' @param additional_ann_cols A character vector: additional columns to separate.
#' @rdname vcfR_to_tidy_separation
#' @export
separate_ann_snpeff <- function(df, meta, additional_ann_cols = c("LOF", "NMD"), ...) {
  trim_values <- function(df, ann_col) {
    df[[ann_col]] <- stringr::str_remove(df[[ann_col]], "^\\(") %>%
      stringr::str_remove("\\)$")
    return(df)
  }

  snpeff_ann_cols <- extract_ann_cols_snpeff(meta, ...)
  df <- separate_ann(df, "ANN", snpeff_ann_cols$ANN, ...)
  if (!is.null(additional_ann_cols) && length(additional_ann_cols) > 0) {
    df_additional <- purrr::map_dfc(
      additional_ann_cols,
      ~ separate_ann(df, ., snpeff_ann_cols[[.]], multi_sep = NULL, value_fn = trim_values, return_into_cols = TRUE)
    )
    df <- dplyr::bind_cols(df, df_additional)
  }
...
#' @rdname vcfR_to_tidy_separation
#' @export
separate_ann_vep <- function(df, meta, ...) {
  vep_ann_cols <- extract_ann_cols_vep(meta, ...)
  separate_ann(df, "CSQ", vep_ann_cols$CSQ, ...)
}

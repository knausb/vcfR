...
chrom <- create_chromR(name="Supercontig_1.100", vcf=vcf, seq=dna, ann=gff, verbose=TRUE)
chrom <- masker(chrom, min_DP = 300, max_DP = 700)
chrom <- proc_chromR(chrom, verbose = TRUE)

```


The function create_chrom() reports the names of the chromosome (supercontig, contig, etc.) so that we can verify that all three data sources contain the same chromosome.
...
```{r, fig.height=7, fig.width=7}
chrom <- proc_chromR(chrom, verbose=FALSE, win.size=1e3)
...
And the variant count per window was summarized during the windowing analysis performed by proc_chrom().

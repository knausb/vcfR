...
The organization of each cell containing a genotype and associated information is specified in colum nine.
...
This means that there are slots for certain data, but any particular software which creates a vcf file does not necessarily use them all.
...
Although, note that the alignment software is not included here because it was used upstream of the vcf creation (aligners typically create \*.SAM or \*.BAM format files).
...
Here, the remaining lines contain INFO and FORMAT specifications which define abbreviations used in the fix and gt portioins of the file.
...
Here, the ID field has not been used, so it consists of missing data.
...
We see that 'DP' is defined both for the INFO column and for the GT portion.


Returning to the INFO column, we see that the value of DP equals 197.
This tell us that this variant was sequenced 197 times over all our samples.

...
The values for genotype qualioty appears to range from 0 to 100 with among sample variability.
...
The values for sequence depth (DP) appear to have a correlation to the genotype qualities.
Their values range from zero (zero has been omitted from the plot in order to log transform the y-axis) to almost 1,000.
...
The algorithm we're going ot use looks for an optimum and not a minimum, so if we multiply by negative one our data will range from negative infinity to zero with zero being optimal.

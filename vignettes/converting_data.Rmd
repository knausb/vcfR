...
This path currently doesn't scale well to large dataset.
...
The function **write.vcf()** can be used to create *.vcf.gz format files (gzipped vcf files) from objects of class vcfR or chromR.
These vcf format files can be used for any downstream analysis which uses vcf format files as input.
...
To address this incompatibility, vcfR2genelight omits loci which include more than two alleles.
...
The vcf format only contains information on variant positions.
...
Once vcf format data are obtained, vcfR provides an interactive way to scrutinize and filter the data.
A number of paths have been provided that take the results of vcf format data from exploration and filtering to downstream analyses.

...
The vcfR package is designed to work with [VCF format](http://www.1000genomes.org/node/101) files.
...
Annotation files (we currently only support [gff](http://sequenceontology.org/resources/gff3.html)), files which contain coordinates for annotations such as start and end points of genes, are tabular and can be read in with typical R functions.
...
The parameter 'ann' should be a [gff](http://sequenceontology.org/resources/gff3.html) and be of class data.frame.
...
Note that VCF files created by different variant calling softwares may or may not have these fields or their ranges may be different.
...
```{r masker proc.chromR, fig.align='center', fig.height=7, fig.width=7}
...
```{r chromoqc3, fig.align='center', fig.height=7, fig.width=7}
record <- 130
buffer=1e3
...
Different softwares will place different types of data to accompany each genotype.
...
```{r heatmap.bp, fig.align='center', fig.height=7, fig.width=7}
heatmap.bp(gq)
```


Each sample is in a column and each variant is in a row.
The sequence begins at the bottom of the plot and extends to the top of the plot.

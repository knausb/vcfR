...
vcfR is a package intended to help visualize, manipulate and quality filter data in vcf format files.
...
The vcfR package is designed to work primarily with [vcf format](http://www.1000genomes.org/node/101) files.
The use of a sequence file ([fasta format](https://en.wikipedia.org/wiki/FASTA_format)) and an annotation file ([gff format](http://www.sequenceontology.org/gff3.shtml)) can provide helpful context, but are not required.
Reading in of the vcf format file can be performed with a function in vcfR.
First we'll locate the example data from the package 'pinfsc50.'
...
The function **read.vcf()** takes the filename you specify and uses it to read the file into a **vcfR** object.
...
Annotation files (e.g., [gff](http://sequenceontology.org/resources/gff3.html)), files which contain coordinates for annotations, such as start and end points of genes, are tabular and can be read in with typical R functions.
...
Thsi can present a challenge to reading these files into R.
...
vcfR was designed to work on individual chromosomes, supercontigs or contigs, depending on the state of your genome.
...
When we examine the output we seen that the name in the vcf file is 'Supercontig_1.50,' while the name in the fasta is 'Supercontig_1.50 of Phytophthora infestans T30-4.'
...
The vcfR object should be of class vcfR, which was most likely created with the function read.vcf().
...
The 'ann' should be a [gff](http://sequenceontology.org/resources/gff3.html) type of table.
...
Presumably, there is some base ploid level at which most of teh genome is sequenced at.
...
Note that vcf files created by different variant calling softwares may or may not have these fields.
...
```{r, fig.align='center', fig.height=7, fig.width=7}
chromoqc(chrom, alpha=10)
```


We can compare this to the raw data by resetting the mask.
...
The last line reimplements our mask after we're done, by using the masker function.


By applying our mask, we retain `r sum(chrom@var.info$mask)` of the `r length(chrom@var.info$mask)` varants, or `r round(100*sum(chrom@var.info$mask)/length(chrom@var.info$mask), digits=1)` percent of the variants.
This is a rather severe mask in terms of attrition rate.
However, if we have effectively identified the high quality fraction of the variants, it may improve our downstream analyses.
...
Or use the annotation file to zoom in on a gene.
...
record <- 130
buffer=1e3
chromoqc(chrom, xlim=c( gff[record,4]-buffer, gff[record,5]+buffer ), dot.alpha = 99)
```




Variants of this function, which are actually all wrappers for the function **chromo()**, create similar plots.
...
Thus far we've only looked at per variant summaries over all samples.
This information is typically found in the fixed portion of a vcf file (the first 8 columns of the file).
Beyond these eight colomns are a flexible number of columns, one for each sample.
Different softwares will place different types of data to accompany each gentype.

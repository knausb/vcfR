...
seq_file <- system.file("extdata", "pinf_sc50.fasta", package = pkg)
...
Reading an entire genome into memory may present a technical challenge when there is a lot of data, the genome is large or there are a lot of samples.
...
Once the data are in memory we can use it to create a **Chrom** object with the function **create.chromR()**.
...
When we examine the output we seen that the name in the vcf file is Supercontig_1.100, while the name in the fasta is Supercontig_1.100 of Phytophthora infestans T30-4.
...
Interpretation of the qualities (QUAL) appear less straightforward and may be bimodal.
...
Once we're satisfied with which variants we want to consider to be of high quality we can process the Chrom object with **proc.chromR()**.

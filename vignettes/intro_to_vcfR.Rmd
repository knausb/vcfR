...
```{r}
#pkg <- "vcfR"
#vcf_file <- system.file("extdata", "pinf_sc1_100_sub.vcf.gz", package = pkg)
#seq_file <- system.file("extdata", "pinf_sc100.fasta", package = pkg)
#seq_file <- system.file("extdata", "pinf_sc100.fasta.gz", package = pkg)
#gff_file <- system.file("extdata", "pinf_sc100.gff", package = pkg)
pkg <- "pinfsc50"
...
The create.chromR() function creates a new Chrom object and populates it with data you provided it.
...
Note that here the names of our three data sources are not identical.
...
This sequence will be inserted into the Chrom object with the function **seq2chromR()**.
...
These can typically read in with base R functions such as read.table.
This table will then be inserted into the Chrom object by **ann2chromR()**.
...
No SNP densities are not found at this point because this data results from windowing analyses performed by **proc_chromR()** (see below).
...
Interpretation of the qualities (QUAL) appear less straightforward and may be clinal from a value of zero.
Filtering on other parameters may reveal a more straight forward path.
Alternatively, you may conclude that this is not an ideal parameter to filter on.
...
(We'll reimplement our mask after we're done with the masker function.)
...
When there are large numbers of variants, it may be more convenient to summarize regions using a windowing analysis.  This can be done with the function **windowize.NM**.


```{r}
gqw <- windowize.NM(gq, pos=chrom@var.info$POS, starts=chrom@win.info$start, ends=chrom@win.info$end)
gqw[1:6,1:8]
```
